...
Aggregates continue to reject multiple source identities when no selector is given. A selected source is returned in the aggregate and as `metric_source` in each observation row. The row's `source` remains the cutoff-resolved event provenance (such as its entry channel), or null for measurements. Period comparisons require the same metric source identity in both aggregates, along with the existing version, unit, scale, and method checks.
