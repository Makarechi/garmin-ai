...
Bounded to 366 days, 200 activities, 200 metadata messages per activity, and 24,000 UTF-8 bytes of output (within the shared agent result budget). Truncation is explicit. This is a first evidence surface: durable cross-activity inventory identity, change-point annotations in comparisons, confirmed sensor-to-sample attribution, body composition, and separate respiration/SpO2 context remain later phases.

Records follow the global FIT parse sequence. Freshness also requires a matching normalized source at the current parser version. Oversized valid responses retain a bounded prefix with explicit truncation instead of returning a generic error; narrow the date range for omitted activities and use activity_details for omitted per-activity records.
